...
export default function ArticlesListe({ articles, auth_user_id }) {
  ...
  const filtrerArticles = () => {
    return articles.filter((article) => {
      ...
      const correspondRecherche = article.titre
        .toLowerCase()
    ...
    <>
      ...
      <section className="mt-40 min-h-screen bg-gray-100 px-4 py-20 font-serif">
        <div className="mx-auto max-w-7xl">
          <h2
            className="mb-10 text-4xl text-center md:text-4xl lg:text-left lg:text-5xl font-extrabold text-orange-400 uppercase tracking-wide leading-tight"
            style={{ fontFamily: "'Merriweather', sans-serif", fontStyle: 'normal', fontWeight: 600 }}
          >
            Zoom sur l’actualité
          </h2>

          <div className="mb-12 flex flex-col lg:flex-row items-center justify-between gap-6 text-black mt-20">
            <div className="relative w-full lg:w-1/2 ">
              <input
                ...
                className="w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-cyan-600 md:py-4"
              />
              <ImSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-cyan-600 pointer-events-none" />
            </div>

            <div className="flex flex-col gap-4 mt-10 w-full md:w-full lg:flex-row lg:w-auto">
              <button
                onClick={() => setFiltre('local')}
                className={`px-6 py-2 text-base 
                  md:px-4 md:py-3 md:text-sm 
                  lg:px-6 lg:py-2 lg:text-base
                  rounded-full border font-semibold transition w-full md:w-full lg:w-auto ${
                  filtre === 'local'
                    ? 'bg-cyan-600 text-white'
                    : 'bg-white text-cyan-700 border-cyan-600 cursor-pointer'
                }`}
              >
                Près de chez vous
              </button>
          ...
          <div className="grid gap-14 md:grid-cols-1 lg:grid-cols-2 mt-20">
